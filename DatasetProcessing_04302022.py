        ...
        for dis_matrix in ("euclidean", "manhattan", "cosine", "precomputed"):
            ...
            if dis_matrix != "precomputed":
                ...
                if (max(clustering.labels_) + 1) == 1: 
                    ...
                    print('Silhouette Score: %.2f' % silhouette_score(df, clustering.labels_, metric = dis_matrix))
                    print('Calinski Harabasz Score: %.2f' % calinski_harabasz_score(df, clustering.labels_) )
                    print('Davies-Bouldin Index: %.2f' % davies_bouldin_score(df, clustering.labels_))
                print(clustering.labels_)
                print("Dimension after embedding:%d\tLinkage: %s\tDistance: %s\tRuntime: %.2fs" %
                    (dimArray.ndim, linkage, dis_matrix, time() - t0))
                case += 1
                print('###########################################################\n')
            else:
                for i in range(3):
                    ...
                    if i == 1:
                        ...
                        dis_name = 'minkowski'
                    # if i == 2:
                    #      dist = DistanceMetric.get_metric('seuclidean', V = np.cov(x))
                    #      dis_name = 'seuclidean'
                    else:
                    ...
                    if (max(clustering.labels_) + 1) == 1: 
                        print('There is only one cluster.')
                    if (len(precomputedMatrix[0] == len(clustering.labels_))):
                        print("The length of labels matches the precomputedmatrix[0] length.")
                    else:
                        print('Silhouette Score: %.2f' % silhouette_score(precomputedMatrix, clustering.labels_, metric = 'precomputed'))
                        print('Calinski Harabasz Score: %.2f' % calinski_harabasz_score(precomputedMatrix, clustering.labels_) )
                        print('Davies-Bouldin Index: %.2f' % davies_bouldin_score(precomputedMatrix, clustering.labels_))
                    print(clustering.labels_)
                    print("Dimension after embedding:%d\tLinkage: %s\tDistance: %s\tRuntime: %.2fs" %
                        (dimArray.ndim, linkage, dis_name, time() - t0))
                    case += 1
                    print('###########################################################\n')